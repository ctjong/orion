...
export interface UserInfo { tokenExpiry:number, name?:string, roles?:string[], domain?:string, id?:string, domainId?: string };
...
export class CompoundCondition implements Condition { operator: string; children:SingleCondition[]; findConditionValue: ((key: string) => string) };

export interface NameValueMap {[key:string]: string};
